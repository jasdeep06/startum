...
Start with [the simple implementation flow](docs/simple-flow.md), which connects what we built, why each part exists and where its code lives. For more detail, see [the code flowcharts](docs/code-flows.md): the overall request flow, plugin registration, Task resolution, Intelligence service execution and ObjectStack preview lifecycle.
